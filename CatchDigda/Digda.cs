        ...
        private async void StartGame()
        {
            ...
            while (true)
            {
                ...
                if (remainingSeconds == 0)
                {
                    ...
                    double ratio = 100.0 / 186.0;
                    final_score = (int)Math.Round(this.score * ratio);

                    string message = string.Format("Time Finished!\n\nScore: {0}\nFinal Score: {1}", score, final_score);
                    MessageBox.Show(message, "게임 종료!", MessageBoxButtons.OK);
