    ...
    public partial class Form1 : Form
    {
        ...
        private void Form1_KeyDown(object sender, KeyEventArgs e)   
        {
            ...
            if (EndGame == true)  // 여기에 메인과 연결 예정
            {
                ...
                currenttotalScore += stageScores[Stage];
                stageCleared[Stage] = true; // 현재 스테이지를 클리어로 표시
                remainingTimes[Stage] = timeCount; // 남은 시간을 저장
                if(Stage==0)
                {
                    MessageBox.Show("OT끝! 이제 학기를 시작합니다!", "학기시작", MessageBoxButtons.OK);

                }
                if (Stage != 0)
                {
                    MessageBox.Show($"{Stage}라운드 완료! 획득 점수: {stageScore}/{stageScores[Stage]}, 총 점수: {totalScore}/{currenttotalScore}", "결과", MessageBoxButtons.OK);
                }
                ...
                else
                {
                    ...
                    if (!allCleared)
                    {
                        ...
                        {
                            finalPercentage = (double)totalScore / stageScores.Skip(1).Sum() * 100; // 첫 번째 스테이지 제외하고 계산
                            MessageBox.Show($"고생하셨어요. 당신의 최종 점수는 {finalPercentage:F2}점입니다.", "최종 성적", MessageBoxButtons.OK);
                            final_score = (int)Math.Round(finalPercentage);
